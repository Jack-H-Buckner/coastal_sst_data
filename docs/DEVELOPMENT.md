...
| `sensor` | a `SensorSpec`, or `None` | set **only** for `OVERPASS_SENSOR` thermal products. Its fields *are* the arguments `load_clearest_overpass` takes. Set `mosaic_same_day=True` when one granule may cover **less than a whole AoI**, so a day's granules are merged (clearest is the base, the rest fill only the cells it left invalid) instead of the clearest being kept alone — never together with `has_footprint`, whose ids restart per granule; the registry check refuses that pairing. |
