...
| 7 | **`store.sweep_scratch` deletes *anyone's* in-flight `.part-*` for the same dest.** | `store.py:93` | **Invariant to preserve:** no two tasks may target the same output path. The `(product, source, aoi)` partition guarantees this — `products._check_registry` already enforces unique `dir` values (`products.py:638`). Add an assertion when building the graph. |
| 8 | **Scratch tag is `pid`+ms** — no thread component. | `store.py:73` | Add `threading.get_ident()`. One line, cheap insurance behind hazard 7. |
