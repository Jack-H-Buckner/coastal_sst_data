...
**Each sensor's `<sensor>_valid` records how its water mask was read.** A sensor's `water` layer means either *water* or *land* depending on the product, and reading it the wrong way round makes the validity mask empty over water — which is not a visible failure, it is a day that looks cloudy and a mosaic that then keeps only one granule. So the assembler does not take the registry's word for it: once per AoI and version tag it compares the layer against the static `landcover_water` mask and uses whichever reading agrees, recording the verdict on the channel as `water_polarity` (`water_is_land` / `water_is_water`), `water_polarity_declared`, `water_polarity_agreement`, and `water_polarity_status`. A status of `corrected` means the detected reading differed from the registry's and the cube used the detected one; `ok` means they agreed; `no_landcover`, `landcover_degenerate`, `ambiguous` and `too_few_cells` all mean the check could not decide and the declared value was kept. A `+water_layer_not_binary` suffix means the layer is not a 0/1 mask at all, which usually means the wrong asset was bound to it. Sensors whose files ship a trustworthy `valid` layer (MODIS) are not checked — they never consult `water`.
